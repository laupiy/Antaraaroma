...
import { Search, SlidersHorizontal, Grid3x3, LayoutList, Star, ChevronDown, X, ArrowUpDown, Package, MessageCircle } from "lucide-react";
...
export function CatalogPage() {
  // ── Dynamic products from localStorage (falls back to default data) ──────
  ...
  // ── Category counts derived from live data ───────────────────────────────
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [allProducts, activeCategory, search, sortKey]);
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "Poppins, sans-serif" }}>
      ...
      <section
        ...
        style={{
          background: "linear-gradient(135deg, #0a0a0a 0%, #111827 45%, #0f2f2e 80%, #0a1a1a 100%)",
        }}
      >
        <div
          className="absolute inset-0 opacity-5"
          style={{
            backgroundImage: `linear-gradient(rgba(39,199,195,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(39,199,195,0.4) 1px, transparent 1px)`,
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-16 text-center">
          <div
            className="flex items-center justify-center gap-2 text-xs mb-5"
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            <Link to="/" className="text-gray-400 hover:text-[#27C7C3] transition-colors">
              Home
            </Link>
          ...
          <h1
            ...
            style={{ fontFamily: "Montserrat, sans-serif", fontWeight: 800, lineHeight: 1.05 }}
          >
            Product{" "}
            <span
              style={{
          ...
          >
            Temukan rangkaian lengkap botol parfum premium, dispenser semprot,
            toples perawatan kulit, roll-on, dan kemasan kosmetik — semua tersedia untuk
            branding kustom dan pemesanan massal.
          </p>
        ...
        <div className="flex gap-8">
          ...
          <aside
            className={`
              ...
              fixed inset-y-0 left-0 z-40 w-72 bg-white shadow-2xl lg:shadow-none
          ...
          >
            <div className="p-6 lg:p-0">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                <h3 className="text-gray-900 font-semibold text-sm mb-4" style={{ fontFamily: "Montserrat, sans-serif" }}>
                  Categories
                </h3>
                <div className="flex flex-col gap-1">
                  {CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm transition-all duration-200 text-left ${
                        activeCategory === cat
                          ? "bg-[#27C7C3] text-white shadow-md shadow-[#27C7C3]/20"
                          : "text-gray-600 hover:bg-gray-50 hover:text-[#27C7C3]"
                      }`}
                      style={{ fontFamily: "Poppins, sans-serif" }}
                    >
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        activeCategory === cat ? "bg-white/25 text-white" : "bg-gray-100 text-gray-400"
                      }`}>
                ...
                <div className="flex flex-col gap-2">
                  {["Glass", "Acrylic", "Aluminum", "Mixed"].map((m) => (
                    <label key={m} className="flex items-center gap-3 cursor-pointer group">
                      ...
                      <span className="text-gray-600 text-sm group-hover:text-[#27C7C3] transition-colors" style={{ fontFamily: "Poppins, sans-serif" }}>
                        {m}
                      </span>
                    </label>
                  ))}
              ...
              >
                ...
                <p className="text-white/70 text-xs" style={{ fontFamily: "Poppins, sans-serif" }}>
                  Chat with us on WhatsApp for bulk pricing and samples.
                </p>
              </a>
            </div>
          </aside>
          ...
          {filterOpen && (
            <div className="fixed inset-0 z-30 bg-black/40 lg:hidden" onClick={() => setFilterOpen(false)} />
          )}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="lg:hidden flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-gray-200 text-gray-600 text-sm font-medium hover:border-[#27C7C3]/30 hover:text-[#27C7C3] transition-all shadow-sm"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                ...
                <span className="text-gray-400 text-sm" style={{ fontFamily: "Poppins, sans-serif" }}>
                  ...
                  {search && (
                    <span className="ml-1">
                      untuk "<span className="text-[#27C7C3]">{search}</span>"
                    </span>
                  )}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-gray-200 text-gray-600 text-sm hover:border-[#27C7C3]/30 hover:text-[#27C7C3] transition-all shadow-sm"
                    style={{ fontFamily: "Poppins, sans-serif" }}
                  >
                    <ArrowUpDown size={14} />
                    {currentSortLabel}
                      ...
                      {SORT_OPTIONS.map((opt) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                            sortKey === opt.value
                              ? "bg-[#27C7C3]/10 text-[#27C7C3] font-medium"
                ...
                <div className="flex rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                  <button
                    ...
                    className={`p-2 transition-colors ${viewMode === "grid" ? "bg-[#27C7C3] text-white" : "text-gray-400 hover:text-[#27C7C3]"}`}
                  >
                    <Grid3x3 size={16} />
                  </button>
                  <button
                    onClick={() => setViewMode("list")}
                    className={`p-2 transition-colors ${viewMode === "list" ? "bg-[#27C7C3] text-white" : "text-gray-400 hover:text-[#27C7C3]"}`}
                  >
                    <LayoutList size={16} />
                  </button>
                </div>
            ...
            {(activeCategory !== "All" || search) && (
              <div className="flex flex-wrap items-center gap-2 mb-5">
                ...
                <button
                  ...
                  className="text-xs text-[#27C7C3] hover:underline"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  Hapus semua
                </button>
            ...
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <h3 className="text-gray-700 font-semibold text-lg mb-2" style={{ fontFamily: "Montserrat, sans-serif" }}>
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-full bg-[#27C7C3] text-white text-sm font-medium"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  Reset Filter
                </button>
            ...
            {viewMode === "list" && filtered.length > 0 && (
              <div className="flex flex-col gap-4">
                {filtered.map((product) => (
                  <ProductListCard key={product.id} product={product} onSelect={() => setSelectedProduct(product)} />
                ))}
              </div>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            <a href="/#contact" className="px-5 py-2.5 rounded-full bg-[#27C7C3] text-white text-sm font-semibold hover:bg-[#1fb3af] transition-all shadow-md hover:shadow-lg hover:scale-105" style={{ fontFamily: "Poppins, sans-serif" }}>
              Minta Penawaran
            </a>
          </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-xl">
          <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col gap-1">
            {links.map((link) => (
              <Link key={link.href} to={link.href} className={`py-3 px-3 rounded-lg text-sm font-medium transition-colors ${link.href === "/catalog" ? "text-[#27C7C3] bg-[#27C7C3]/5" : "text-gray-700 hover:text-[#27C7C3] hover:bg-[#27C7C3]/5"}`} style={{ fontFamily: "Poppins, sans-serif" }} onClick={() => setMobileOpen(false)}>
                {link.label}
              </Link>
            ))}
            <a href="/#contact" className="mt-2 py-3 px-5 rounded-full bg-[#27C7C3] text-white text-sm font-semibold text-center" style={{ fontFamily: "Poppins, sans-serif" }} onClick={() => setMobileOpen(false)}>
              Minta Penawaran
            </a>
          </div>
        </div>
  ...
  return (
    <div className="group bg-white rounded-3xl overflow-hidden border border-gray-100 hover:border-[#27C7C3]/25 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5 cursor-pointer flex flex-col" onClick={onSelect}>
      <div className="relative overflow-hidden h-56 bg-gray-50">
        ...
        {product.badge && (
          <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold text-white shadow" style={{ background: product.badgeColor || "#27C7C3" }}>
            {product.badge}
          </span>
        )}
        {!product.badge && product.isNew && (
          <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold text-white bg-emerald-500 shadow">New</span>
        )}
        <div className="absolute inset-x-3 bottom-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
          <button className="w-full py-2.5 rounded-xl bg-white/95 backdrop-blur-sm text-gray-900 text-xs font-semibold hover:bg-[#27C7C3] hover:text-white transition-colors shadow-sm" style={{ fontFamily: "Poppins, sans-serif" }}>
            Lihat Detail →
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center gap-1.5">
            <div className="flex gap-0.5">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} size={11} className={i < Math.floor(product.rating) ? "text-amber-400 fill-amber-400" : "text-gray-200 fill-gray-200"} />
              ))}
            </div>
...
function ProductListCard({ product, onSelect }: { product: Product; onSelect: () => void }) {
  return (
    <div className="group bg-white rounded-2xl border border-gray-100 hover:border-[#27C7C3]/25 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer flex gap-5 p-4 hover:-translate-y-0.5" onClick={onSelect}>
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        <div className="flex items-start justify-between gap-4">
          ...
          <button className="shrink-0 px-4 py-2 rounded-xl bg-[#27C7C3]/10 text-[#27C7C3] text-xs font-semibold hover:bg-[#27C7C3] hover:text-white transition-all hidden sm:block" style={{ fontFamily: "Poppins, sans-serif" }}>
            Lihat Detail
          </button>
        </div>
...
function SpecPill({ label }: { label: string }) {
  return (
    <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 text-xs" style={{ fontFamily: "Poppins, sans-serif" }}>{label}</span>
  );
}
...
function FilterChip({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#27C7C3]/10 text-[#27C7C3] text-xs font-medium" style={{ fontFamily: "Poppins, sans-serif" }}>
      {label}
      <button onClick={onRemove} className="hover:text-[#1aa8a5]"><X size={12} /></button>
    </span>
  );
}
