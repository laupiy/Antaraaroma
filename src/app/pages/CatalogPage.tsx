...
import { CATALOG_PRODUCTS, type Category, type Product } from "../data/catalogData";
...
const CATEGORY_COUNTS: Record<string, number> = CATALOG_PRODUCTS.reduce(
  (acc, p) => {
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-16 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#27C7C3]/15 border border-[#27C7C3]/30 mb-5">
            ...
            <span
              ...
              style={{ fontFamily: "Poppins, sans-serif" }}
            >
              {CATALOG_PRODUCTS.length} Produk Tersedia
            </span>
          </div>
                ...
                <div className="flex flex-col gap-2">
                  {["Glass", "Acrylic", "Aluminum", "Mixed"].map((m) => (
                    <label key={m} className="flex items-center gap-3 cursor-pointer group">
                      ...
                      <span
                        className="text-gray-600 text-sm group-hover:text-[#27C7C3] transition-colors"
                        style={{ fontFamily: "Poppins, sans-serif" }}
                      >
                        {m}
                      </span>
                  ...
                  <button
                    ...
                    className={`p-2 transition-colors ${
                      viewMode === "grid"
                        ...
                        : "text-gray-400 hover:text-[#27C7C3]"
                    }`}
                  >
                    <Grid3x3 size={16} />
                  </button>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {links.map((link) => (
              <Link
                key={link.href}
                to={link.href}
                className={`text-sm font-medium transition-colors hover:text-[#27C7C3] ${
                  link.href === "/catalog" ? "text-[#27C7C3]" : "text-gray-600"
                }`}
                style={{ fontFamily: "Poppins, sans-serif" }}
              >
                {link.label}
                {link.href === "/catalog" && (
                  <span className="ml-1.5 inline-block w-1.5 h-1.5 rounded-full bg-[#27C7C3] align-middle" />
                )}
              </Link>
            ))}
            <a
              href="/#contact"
              className="px-5 py-2.5 rounded-full bg-[#27C7C3] text-white text-sm font-semibold hover:bg-[#1fb3af] transition-all shadow-md hover:shadow-lg hover:scale-105"
              style={{ fontFamily: "Poppins, sans-serif" }}
            >
              Minta Penawaran
            </a>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-xl">
          <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col gap-1">
            {links.map((link) => (
              <Link
                ...
                className={`py-3 px-3 rounded-lg text-sm font-medium transition-colors ${
                  link.href === "/catalog"
                    ? "text-[#27C7C3] bg-[#27C7C3]/5"
                    : "text-gray-700 hover:text-[#27C7C3] hover:bg-[#27C7C3]/5"
                }`}
                style={{ fontFamily: "Poppins, sans-serif" }}
            ...
            <a
              href="/#contact"
              className="mt-2 py-3 px-5 rounded-full bg-[#27C7C3] text-white text-sm font-semibold text-center"
              style={{ fontFamily: "Poppins, sans-serif" }}
              onClick={() => setMobileOpen(false)}
            >
              Minta Penawaran
...
function ProductCard({ product, onSelect }: { product: Product; onSelect: () => void }) {
  return (
    <div
      className="group bg-white rounded-3xl overflow-hidden border border-gray-100 hover:border-[#27C7C3]/25 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5 cursor-pointer flex flex-col"
      onClick={onSelect}
    >
      {/* Image */}
      <div className="relative overflow-hidden h-56 bg-gray-50">
        ...
        {!product.badge && product.isNew && (
          <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold text-white bg-emerald-500 shadow">
            New
          </span>
        )}
        ...
        <div className="absolute inset-x-3 bottom-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
          <button
            className="w-full py-2.5 rounded-xl bg-white/95 backdrop-blur-sm text-gray-900 text-xs font-semibold hover:bg-[#27C7C3] hover:text-white transition-colors shadow-sm"
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            Lihat Detail →
          </button>
      ...
      <div className="p-5 flex flex-col flex-1">
        {/* Category */}
        <span
          className="text-[#27C7C3] text-xs font-semibold uppercase tracking-wider mb-1.5"
          style={{ fontFamily: "Poppins, sans-serif" }}
        >
          {product.category}
        </span>
        ...
        {/* Name */}
        <h3
          className="text-gray-900 text-sm font-semibold mb-2 group-hover:text-[#27C7C3] transition-colors line-clamp-2"
          style={{ fontFamily: "Montserrat, sans-serif" }}
        >
          {product.name}
        </h3>
...
function ProductListCard({ product, onSelect }: { product: Product; onSelect: () => void }) {
  return (
    <div
      className="group bg-white rounded-2xl border border-gray-100 hover:border-[#27C7C3]/25 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer flex gap-5 p-4 hover:-translate-y-0.5"
      onClick={onSelect}
    >
      {/* Image */}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h3
              className="text-gray-900 font-semibold mt-0.5 mb-2 group-hover:text-[#27C7C3] transition-colors"
              style={{ fontFamily: "Montserrat, sans-serif" }}
            >
              {product.name}
            </h3>
          ...
          <button
            className="shrink-0 px-4 py-2 rounded-xl bg-[#27C7C3]/10 text-[#27C7C3] text-xs font-semibold hover:bg-[#27C7C3] hover:text-white transition-all hidden sm:block"
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            Lihat Detail
          </button>
...
function FilterChip({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span
      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#27C7C3]/10 text-[#27C7C3] text-xs font-medium"
      style={{ fontFamily: "Poppins, sans-serif" }}
    >
      {label}
      <button onClick={onRemove} className="hover:text-[#1aa8a5]">
        <X size={12} />
      </button>
    </span>
  );
